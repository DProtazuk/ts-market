<style>
    ...
    .layout .sidebar {
        width: 200px;
        min-width: 200px;
        -webkit-transition: width, left, right, 0.2s;
...
<script>
    function status_menu() {
        if(localStorage.getItem('ns_market') == "1") {
            localStorage.setItem('ns_market', 0);
        }
        else localStorage.setItem('ns_market', 1);
    }

    function status_menu_markup() {
        if ($(window).width() > 993) {
            if (localStorage.getItem('ns_market') == 1) {
